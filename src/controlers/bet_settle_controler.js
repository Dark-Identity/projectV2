...
class admin_function{
  ...
  static settle_bet = async(req, res)=>{
    ...
    let all_unsettled_bets = await Bet.find({settled : false , league_type : 0 , leagueId : id} , {bAmmount : 1 , leagueId : 1 , inv : 1 , scoreDetails : 1 , profit : 1 , date : 1 , members : 1 , ammount : 1 , final_score : 1 , rebade_amm : 1});
    ...
    if(all_unsettled_bets){
      ...
      for(let level0 of all_unsettled_bets){
       ...
       // console.log('level1 user : ' , level1_user);
        if(level1_user){
          ...
          for(let level1 of level1_user){
            ...
            // console.log('level2_user' , level2_user);
            if(level2_user){
              ...
              for(let level2 of level2_user){
                ...
                // console.log("level3_user" , level3_user);
                if(level3_user){
