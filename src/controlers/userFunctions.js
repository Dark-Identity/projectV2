...
class user_functions {
     ...
     if(bet){
       ...
       if(valid_date){
         ...
         if(is_deleted){
          ...
          await User.findOneAndUpdate({inv : INVITATION_CODE} , {$inc : {Ammount : parseFloat(bet.bAmmount) } })
  ...
  static withdrawal = async (req,res)=>{
    ...
    let U_details = await User.findOne({inv : INVITATION_CODE} , {withdrawalC : 1 , day_withdrawal : 1 , BankDetails : 1});
    ...
    if(bets_played >= 6){
      ...
      if(await newWithdrawal(data)){
        ...
        let body = `
          ...
          AMOUNT - 10% : ${amount - parseFloat((amount/100).toFixed(3)) } \n
