...
function calc_available() {
  ...
  if(!value){
    ...
    let profit = select('#profit').replace(/\d%/ , '');
...
selectAll('.bet_btns > span').forEach((item, i) => {
  item.addEventListener('click' , ()=>{
    if(i !== 4){
      ...
      let value2 = parseFloat(select('.s_balance').replace(/\D/ , ''));
...
select('#league_match_btn').addEventListener('click' , ()=>{
  ...
  select('#virtual_match_btn').style.background = '#ff2f2f';
...
select('#virtual_match_btn').addEventListener('click' , ()=>{
  ...
  select('#league_match_btn').style.background = '#ff2f2f';
...
selectAll('.aff-new-register').forEach((item, i) => {
    item.addEventListener('click' , ()=>{
      ...
      select('#affiliate_page').style.scrollTo(0,0);
...
function set_user_data(info){
  ...
  selectAll('.s_balance').forEach((item, i) => {
    item.innerText = '₹ ' + info['balance'].toFixed(3);
  });
...
let body = `<h3>${data['league']}</h3>
  ...
  <div class="second-match-card">
    ...
    <span>
      <h4>PROFIT</h4>
      <h5 class="lime">5.5%</h5>
      <h6>AVAILABLE</h6>
    </span>
    <span>
      <h4>AMOUNT</h4>
      <h5>${data['bAmmount']}</h5>
      <h6 class="lime">100</h6>
    </span>
  </div>`;
...
// setting every data to the child;
  ...
  select('#s_tot_profit').innerText = user_profit;
  select('#s_tot_bets').innerText = total_bets_played;
  select('#s_tot_deposit').innerText = user_amount;
